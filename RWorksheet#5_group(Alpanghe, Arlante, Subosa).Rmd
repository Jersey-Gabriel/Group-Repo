...
# 3. Get the links for individual TV show pages
...
# Debugging: Check if links are successfully extracted
print(length(links))  # Print the number of links
print(head(links))  # Print the first few links

# 4. Get details for each individual TV show
show_data <- lapply(links, function(link) {
  complete_link <- paste0("https://imdb.com", link)
  
  # Print the link being processed (debugging)
  print(complete_link)
  
  # Get user review page link
  usrv_link <- read_html(complete_link)
  usrv_link_page <- usrv_link %>%
    html_nodes('a.isReview') %>%
    html_attr("href")
  
  # Debugging: Check if user review page link exists
  print(usrv_link_page)

  # Get critic reviews (handle cases where this may not exist)
  critic <- usrv_link %>%
    html_nodes("span.score") %>%
    html_text()
  
  # Handle missing critic review gracefully
  critic_df <- data.frame(Critic_Reviews = ifelse(length(critic) >= 2, critic[2], NA), stringsAsFactors = FALSE)
...
```{r}
  # Get user reviews (handle missing data gracefully)
  usrv_count <- NULL
  ...
  # Return the data for this show
  return(data.frame(
    Show_Link = complete_link, 
    User_Reviews = ifelse(length(usrv_count) > 0, usrv_count, NA), 
...
# Debugging: Check if show_data contains results
print(length(show_data))  # Print the number of results
print(head(show_data))  # Print the first few entries of show_data
```
